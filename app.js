...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function renderHomeSearchResults() {
        ...
        if (minifigs.length === 0) {
            ...
            minifigs.forEach((item, index) => {
                const card = document.createElement("div");
                card.className = "gallery-card";
                card.setAttribute("data-id", item.minifig_num);
                card.innerHTML = `
                    <div class="gallery-card-img">
                        <img src="https://cdn.rebrickable.com/media/sets/${item.minifig_num}.jpg" alt="${item.name}" loading="${index < 6 ? 'eager' : 'lazy'}" fetchpriority="${index < 6 ? 'high' : 'auto'}" decoding="async"
            ...
            sets.forEach((set, index) => {
                ...
                card.innerHTML = `
                    <div class="set-card-gallery-img">
                        <img src="${imgUrl}" alt="${set.name}" loading="${index < 6 ? 'eager' : 'lazy'}" fetchpriority="${index < 6 ? 'high' : 'auto'}" decoding="async"
    ...
    function showScanResults() {
        ...
        if (isPartMatch && scannedPartInfo) {
            ...
            const matchScoreText = scannedPartInfo.score
                ? `模型置信度 ${(scannedPartInfo.score * 100).toFixed(1)}%`
                : '零件匹配';
        ...
        if (item2 || item3) {
            ...
            [item2, item3].forEach((item, index) => {
                if (!item) return;
                const percent = item.score ? `模型 ${(item.score * 100).toFixed(1)}%` : '候选结果';
